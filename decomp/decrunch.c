...
static void __not_in_flash("up_SetInput") up_SetInput(const unsigned char *data) 
{
	up_Data = data;
	up_Mask = 0x80;
	up_Byte = 0;
...
static uint8_t __not_in_flash("up_GetBits") up_GetBits(int bits) 
{
	uint8_t val = 0;
...
static __not_in_flash("up_GetValue") uint8_t up_GetValue(void) 
{
	uint8_t i = 0;
...
int __not_in_flash("UnPack") UnPack(int loadAddr, unsigned char *data, unsigned char * outBuffer, int outbufsize) 
{
	int outPointer = 0;
