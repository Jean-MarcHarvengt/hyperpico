...
;        ; -------------------
;        LD HL, posoffset        ; print pointer track
;        ADD HL, BC
;        LD A, (HL)
;        LD HL,REG_VIDBASE+0*64
;        LD BC,(tmpregofst)
;        ADD HL,BC
;        CALL printhex
;        LD BC,(tmpchnofst)
;        ; -------------------
;        ; -------------------
;        LD HL, patoffset        ; print pointer pattern
;        ADD HL, BC
;        LD A, (HL)
;        LD HL,REG_VIDBASE+1*64
;        LD BC,(tmpregofst)
;        ADD HL,BC
;        CALL printhex
;        LD BC,(tmpchnofst)
...
getnotedata:
;        ; -------------------
;        LD HL,REG_VIDBASE+2*64 ; print pattern
;        LD BC,(tmpregofst)
;        ADD HL,BC
;        CALL printhex
;        LD BC,(tmpchnofst)
...
;bit5 signals no release (see soundwork)
...
;for no release
        ...
        LD HL, savelnthcc       ;check for no release
        ADD HL, BC
        LD A, (HL)
        AND $20                 ;specified
        JR NZ, vibrato
...
vibrato:
        ...
        LD E, A
        LD D, 0
        LD HL, instr + 7         ;get instr fx byte
        ADD HL, DE
        LD A, (HL)
        LD (instrfx), A

        LD HL, instr + 6        ;get pulse speed
        ADD HL, DE
        LD A, (HL)
        LD (pulsevalue), A

        LD HL, instr + 5        ;get vibrato depth
        ADD HL, DE
        LD A, (HL)
        LD (vibrdepth), A
        JP Z, pulsework

        LD A, (counter)         ;this is clever!!
        AND 7                   ;the counter's turned
        CP 4                    ;into an oscillating
        JR C, l9                ;value (01233210)
        XOR 7
l9:     LD (oscilatval), A

        LD HL, notenum          ;get base note
        ADD HL, BC              ;note*2
        LD A, (HL)
        ADD A, A
        LD E, A
        LD D, 0
        LD HL, frequenzlo + 2   ;get diff btw note
        ADD HL, DE              ;and note+1 frequency
        LD A, (HL)
        LD HL, frequenzlo
        ADD HL, DE
        SUB (HL)
...
l10:    SRL A                   ; Divide delta values by 2 for each vibrdepth
        LD H, A
        LD A, (tmpvdiflo)
        RRA
        LD (tmpvdiflo), A
        ...
        LD HL, frequenzlo       ;save note frequency
        ADD HL, DE
        LD A, (HL)
        LD (tmpvfrqlo), A
        LD HL, frequenzhi
        ...
        LD HL, savelnthcc       ;no vibrato if note
        ADD HL, BC
        LD A, (HL)
...
l11:    DEC E                   ;depending on the osc
        JP M, l12               ;value, add the vibr

        LD A, (tmpvfrqlo)       ;freq that many times
        LD HL, tmpvdiflo        ;to the base freq
        ADD A, (HL)
        LD (tmpvfrqlo), A
        LD A, (tmpvfrqhi)
        LD HL, tmpvdifhi
...
l12:    LD DE, (tmpregofst)     ;save the final
        LD A, (tmpvfrqlo)       ;frequencies
        LD HL, REG_SIDBASE + $00
        ADD HL, DE
        LD (HL), A
        LD A, (tmpvfrqhi)
...
pulsework:
        ...
        LD A, (pulsevalue)      ;check for pulsework
        AND A
        JP Z, portamento        ;needed this instr

        LD A, (instnumby8)      ; instrument offset in DE
        LD E, A
        LD D, 0
        
        LD HL, pulsedelay       ;pulsedelay-1
        ADD HL, BC
        DEC (HL)
        JP P, portamento
        ...
        LD A, (pulsevalue)      ;restrict pulse speed
        AND $E0                 ;from $00-$1f
        LD (pulsespeed), A


        LD HL, pulsedir         ;pulsedir 0 is up and
        ADD HL, BC
        LD A, (HL)
        AND A
        JR NZ, pulsedown        ;1 is down


        LD A, (pulsespeed)      ;pulse width up
        LD IX, instr + 0        ;add the pulsespeed to the pulse width
        ADD IX, DE      
        ADD A, (IX + 0)
        LD H, A                 ; Temporary push 
        ...
        ADC A, (IX + 0)
        AND $0F
        LD L, A                 ; Temporary push

        PUSH HL
        PUSH HL

        CP $0E                  ;go pulsedown when
        JR NZ, dumpulse         ;the pulse value
        LD HL, pulsedir         ;reaches max ($0exx)
        ADD HL, BC
        INC (HL)
        JR dumpulse

pulsedown:
        LD IX, instr + 0        ;pulse width down
        ADD IX, DE
        LD A, (IX + 0)
        LD HL, pulsespeed
        SUB (HL)                ;sub the pulsespeed from the pulse width
        LD H, A                 ; Temporary push
        ...
        CP $08                  ;go pulseup when
        JR NZ, dumpulse         ;the pulse value
        LD HL, pulsedir         ;reaches min ($08xx)
        ADD HL, BC
        DEC (HL)
...
portamento:
        ...
        LD HL, portaval         ;check for portamento
        ADD HL, BC
        LD A, (HL)
        AND A
        JR Z, drums             ;none
        ...
        LD HL, portaval         ;bit0 signals up/down
        ADD HL, BC
        LD A, (HL)
        AND $01
        JR Z, portup
        ...
        LD HL, savefreqlo       ; Portamento down
        ADD HL, BC
        LD A, (HL)
        LD HL, tempstore
        SUB (HL)                ;sub portaval from
        LD HL, savefreqlo       ;current frequency
        ADD HL, BC
        LD (HL), A
        LD HL, REG_SIDBASE + $00
        ADD HL, DE
...
portup:
        LD HL, savefreqlo       ;portamento up
        ADD HL, BC
        LD A, (HL)
        LD HL, tempstore
        ADD A, (HL)             ;add portval to
        LD HL, savefreqlo       ;current frequency
        ADD HL, BC
        LD (HL), A
        LD HL, REG_SIDBASE + $00
...
drums:
        ...
        LD A, (instrfx)         ;check if drums
        AND $01                 ;needed this instr
        JR Z, skydive

        LD HL, savefreqhi       ;don't bother if freq
        ADD HL, BC              ;can't go any lower
        LD A, (HL)
        AND A
        JR Z, skydive

        LD HL, lengthleft       ;or if the note has
        ADD HL, BC              ;finished
        LD A, (HL)
        AND A
        JR Z, skydive

        LD HL, savelnthcc       ;check if this is the
        ADD HL, BC              ;first vbl for this
        LD A, (HL)              ;instrument-note
        AND $1F
        DEC A
        LD HL, lengthleft
        ADD HL, BC
        ...
        LD HL, savefreqhi       ;not the first time
        ADD HL, BC              ;so dec freqhi for
        LD A, (HL)              ;drum sound
        DEC (HL)                
        LD HL, REG_SIDBASE + $01
        ADD HL, DE
        ...
        LD HL, voicectrl        ;if ctrlreg is 0 then
        ADD HL, BC              ;noise is used always
        LD A, (HL)
        AND $FE
        JR NZ, dumpctrl

firstime:
        LD HL, savefreqhi       ;noise is used for
        ADD HL, BC              ;the first vbl also
        LD A, (HL)
        LD HL, REG_SIDBASE + $01
        ADD HL, DE
        LD (HL), A
...
skydive:
        ...
        LD A, (instrfx)         ;check if skydive
        AND $02                 ;needed this instr
        JR Z, octarp

        LD A, (counter)         ;every 2nd vbl
        AND $01
        JR Z, octarp

        LD HL, savefreqhi       ;check if skydive
        ADD HL, BC
        LD A, (HL)
        AND A
        JR Z, octarp            ;already complete

        LD A, (HL)
        DEC (HL)                ;decr and save the
        LD HL, REG_SIDBASE + $01 ;high byte freq
        ADD HL, DE
        LD (HL), A
...
octarp:
        ...
        LD A, (instrfx)         ;check if arpt needed
        AND $04
        JR Z, loopcont

        LD A, (counter)         ;only 2 arpt values
        AND $01
        JR Z, l13

        LD HL, notenum          ;odd, note+12
        ADD HL, BC
        LD A, (HL)
        ADD A, $0C
        JR l14

l13:    LD HL, notenum          ;even, note
        ADD HL, BC
        LD A, (HL)

l14:    ADD A, A                 ;dump the corresponding
        LD E, A                 ;frequencies
        LD D, 0
        LD HL, frequenzlo
        ADD HL, DE
...
dumpsid:
        LD A, (REG_SIDBASE+0)
        LD HL,REG_VIDBASE+0*2
        CALL printhex
        LD A, (REG_SIDBASE+1)
        LD HL,REG_VIDBASE+1*2
        CALL printhex
        LD A, (REG_SIDBASE+2)
        LD HL,REG_VIDBASE+2*2
        CALL printhex
        LD A, (REG_SIDBASE+3)
        LD HL,REG_VIDBASE+3*2
        CALL printhex
        LD A, (REG_SIDBASE+4)
        LD HL,REG_VIDBASE+4*2
        CALL printhex
        LD A, (REG_SIDBASE+5)
        LD HL,REG_VIDBASE+5*2
        CALL printhex
        LD A, (REG_SIDBASE+6)
        LD HL,REG_VIDBASE+6*2
        CALL printhex
        LD A, (REG_SIDBASE+7)
        LD HL,REG_VIDBASE+7*2
        CALL printhex
        LD A, (REG_SIDBASE+8)
        LD HL,REG_VIDBASE+8*2
        CALL printhex
        LD A, (REG_SIDBASE+9)
        LD HL,REG_VIDBASE+9*2
        CALL printhex
        LD A, (REG_SIDBASE+10)
        LD HL,REG_VIDBASE+10*2
        CALL printhex
        LD A, (REG_SIDBASE+11)
        LD HL,REG_VIDBASE+11*2
        CALL printhex
        LD A, (REG_SIDBASE+12)
        LD HL,REG_VIDBASE+12*2
        CALL printhex
        LD A, (REG_SIDBASE+13)
        LD HL,REG_VIDBASE+13*2
        CALL printhex
        LD A, (REG_SIDBASE+14)
        LD HL,REG_VIDBASE+14*2
        CALL printhex
        LD A, (REG_SIDBASE+15)
        LD HL,REG_VIDBASE+15*2
        CALL printhex

        LD A, (REG_SIDBASE+16)
        LD HL,REG_VIDBASE+16*2
        CALL printhex
        LD A, (REG_SIDBASE+17)
        LD HL,REG_VIDBASE+17*2
        CALL printhex
        LD A, (REG_SIDBASE+18)
        LD HL,REG_VIDBASE+18*2
        CALL printhex
        LD A, (REG_SIDBASE+19)
        LD HL,REG_VIDBASE+19*2
        CALL printhex
        LD A, (REG_SIDBASE+20)
        LD HL,REG_VIDBASE+20*2
        CALL printhex
        LD A, (REG_SIDBASE+21)
        LD HL,REG_VIDBASE+21*2
        CALL printhex
        LD A, (REG_SIDBASE+22)
        LD HL,REG_VIDBASE+22*2
        CALL printhex
        LD A, (REG_SIDBASE+23)
        LD HL,REG_VIDBASE+23*2
        CALL printhex
        LD A, (REG_SIDBASE+24)
        LD HL,REG_VIDBASE+24*2
        CALL printhex
        LD A, (REG_SIDBASE+25)
        LD HL,REG_VIDBASE+25*2
        CALL printhex
        LD A, (REG_SIDBASE+26)
        LD HL,REG_VIDBASE+26*2
        CALL printhex
        LD A, (REG_SIDBASE+27)
        LD HL,REG_VIDBASE+27*2
        CALL printhex
        LD A, (REG_SIDBASE+28)
        LD HL,REG_VIDBASE+28*2
        CALL printhex
        LD A, (REG_SIDBASE+29)
        LD HL,REG_VIDBASE+29*2
        CALL printhex
        LD A, (REG_SIDBASE+30)
        LD HL,REG_VIDBASE+30*2
        CALL printhex
        LD A, (REG_SIDBASE+31)
        LD HL,REG_VIDBASE+31*2
        CALL printhex        
        RET
